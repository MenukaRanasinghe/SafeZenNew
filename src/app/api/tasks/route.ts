...
const validStatuses = ['Not Assigned', 'In Progress', 'Completed'];
...
export async function POST(request: Request) {
    try {
        ...
        if (!validStatuses.includes(status)) {
            return NextResponse.json({ error: `Invalid status value. Allowed values are: ${validStatuses.join(', ')}` }, { status: 400 });
        }
